<?php
    ...
    $sql = mysqli_query($con, "SELECT * FROM aims_land_category_run_no WHERE category = '$category'");
    ...
    for ($i = 0; $i < $quantity; $i++) {
        ...
        $sqlDuplicateCheck = "SELECT COUNT(*) as count FROM aims_property_land WHERE name = '$name'";
        ...
        // Insert data into the aims_property_land table
        $sqlAsset = "INSERT INTO aims_property_land 
        (name, asset_tag, category, status, price, land_area_size, land_area_price, remarks, address, document) 
        VALUES 
        ('$name', '$asset_tag', '$category', '$status', '$price', '$land_area_size', '$land_area_price','$remarks', '$address', '$document')";
            ...
            for ($j = 0; $j < $pictureCount; $j++) {
                ...
                if (isset($_FILES['picture']['name'][$j]) && $_FILES['picture']['size'][$j] > 0) {
                    ...
                    if (copy($picture_tmp, $picture_destination)) {
                        ...
                        $picturesForCurrentAsset[] = "include/upload/picture/property/land/" . $asset_tag . "/" . $picture_name;
                    }
                }
            }
        }
    ...
    if ($queryAsset) {
        ...
        $update_running_no = mysqli_query($con, "UPDATE aims_land_category_run_no SET next_no = '$next_no' WHERE category = '$category'");
        echo "true";
    } else {
        echo "false";
    }
