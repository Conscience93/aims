<?php
    ...
    $room = escapeInput($con, $_POST['room']);
    ...
    $furnish = escapeInput($con, $_POST['furnish']);
    $bathroom = escapeInput($con, $_POST['bathroom']);
    $parking = escapeInput($con, $_POST['parking']);
    ...
    $sql = mysqli_query($con, "SELECT * FROM aims_residential_category_run_no WHERE category = '$category'");
    ...
    for ($i = 0; $i < $quantity; $i++) {
        ...
        $sqlDuplicateCheck = "SELECT COUNT(*) as count FROM aims_property_residential WHERE name = '$name'";
        ...
        $target_directory_document = "../../../include/upload/document/property/residential/" . $asset_tag . "/";
        ...
        if (isset($_FILES['document']) && $_FILES['document']['error'] === UPLOAD_ERR_OK) {
            ...
            if (copy($document_tmp, $document_destination)) {
                ...
                $document = "include/upload/document/residential/" . $asset_tag . "/" . $document_name;
            }
        }

        // Insert data into the aims_property_residential table
        $sqlAsset = "INSERT INTO aims_property_residential 
        (name, asset_tag, category, status, price, room, built_up_price, built_up_size, land_area_size, land_area_price, furnish, bathroom, parking, remarks, address, document) 
        VALUES 
        ('$name', '$asset_tag', '$category', '$status', '$price', '$room', '$built_up_price', '$built_up_size', '$land_area_size', '$land_area_price', '$furnish', '$bathroom', '$parking', '$remarks', '$address', '$document')";
        ...
        $target_directory_picture = "../../../include/upload/picture/property/residential/" . $asset_tag . "/";
            ...
            for ($j = 0; $j < $pictureCount; $j++) {
                ...
                if (isset($_FILES['picture']['name'][$j]) && $_FILES['picture']['size'][$j] > 0) {
                    ...
                    if (copy($picture_tmp, $picture_destination)) {
                        ...
                        $picturesForCurrentAsset[] = "include/upload/picture/property/residential/" . $asset_tag . "/" . $picture_name;
                    }
                }
            }
        }
    ...
    if ($queryAsset) {
        ...
        $update_running_no = mysqli_query($con, "UPDATE aims_residential_category_run_no SET next_no = '$next_no' WHERE category = '$category'");
        echo "true";
    } else {
        echo "false";
    }
