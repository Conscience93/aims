<?php
    ...
    $room = isset($_POST['room']) ?  escapeInput($con, $_POST['room']): null;
    $built_up_price = isset($_POST['built_up_price']) ?  escapeInput($con, $_POST['built_up_price']): null;
    $built_up_size = isset($_POST['built_up_size']) ?  escapeInput($con, $_POST['built_up_size']): null;
    ...
    $furnish = isset($_POST['furnish']) ?  escapeInput($con, $_POST['furnish']): null;
    $bathroom = isset($_POST['bathroom']) ?  escapeInput($con, $_POST['bathroom']): null;
    $parking = isset($_POST['parking']) ?  escapeInput($con, $_POST['parking']): null;
    ...
    // Get existing link
    $sqlLink = mysqli_query($con, "SELECT * FROM aims_property_residential WHERE id = '$id'");
    ...
    if ($resultLink["document"]) {
        ...
        if (move_uploaded_file($document_tmp, $document_destination)) {
            $document = "include/upload/document/property/residential/" . $asset_tag . "/" . $document_name;
        } else {
    ...
    $sql_asset = "UPDATE aims_property_residential SET
        name = '$name',
        category = '$category',
        price = '$price',
        room = '$room',
    ...
    if (isset($_FILES['picture']) && $_FILES['picture']['name'][0] != "") {
        ...
        for ($i = 0; $i < $pictureCount; $i++) {
            ...
            if (move_uploaded_file($picture_tmp, $picture_destination)) {
                ...
                $picture[] = "include/upload/picture/property/residential/" . $asset_tag . "/" . $picture_name;
            }
        }
    }
