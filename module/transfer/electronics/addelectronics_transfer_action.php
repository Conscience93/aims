...
// Insert data into the aims_transfer_electronics table
...
if ($queryAsset) {
    // Update the current location in aims_electronics table
    $updateSql = "UPDATE aims_electronics SET branch = '$transfer_branch', department = '$transfer_department', location = '$transfer_location', status = 'TRANSFER' WHERE asset_tag = '$asset_tag'";
    $queryUpdate = mysqli_query($con, $updateSql);
    
    if ($queryUpdate) {
        echo "true";
    } else {
        echo "Update in aims_electronics failed: " . mysqli_error($con);
    }
} else {
    echo "Insert into aims_transfer_electronics failed: " . mysqli_error($con);
}
?>
