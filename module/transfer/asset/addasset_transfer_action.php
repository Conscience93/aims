...
// Insert data into the aims_transfer_asset table
$sql = "INSERT INTO aims_transfer_asset 
...
if ($queryAsset) {
    // Update the current location in aims_asset table
    $updateSql = "UPDATE aims_asset SET branch = '$transfer_branch', department = '$transfer_department', location = '$transfer_location', status = 'TRANSFER' WHERE asset_tag = '$asset_tag'";
    $queryUpdate = mysqli_query($con, $updateSql);
    
    if ($queryUpdate) {
        echo "true";
    } else {
        echo "Update in aims_asset failed: " . mysqli_error($con);
    }
} else {
    echo "Insert into aims_transfer_asset failed: " . mysqli_error($con);
}
?>
