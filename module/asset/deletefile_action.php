...
$sqlGetAsset = "SELECT * FROM aims_asset WHERE id = '$id'";
...
if ($resultGetAsset && mysqli_num_rows($resultGetAsset) > 0) {
    ...
    if (!empty($filePath)) {
        ...
        if (!file_exists($filePath)) {
            ...
            $sqlUpdateField = "UPDATE aims_asset SET $fieldName = '' WHERE id = '$id'";
            $resultUpdateField = mysqli_query($con, $sqlUpdateField);

            if ($resultUpdateField) {
                echo "File deleted and database updated successfully."; 

                echo "<script>window.location.href = 'http://localhost/aims/editasset?id=".$row["id"]."';</script>";
            } else {
