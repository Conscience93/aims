...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_FILES['import'])) {
    ...
    if (move_uploaded_file($_FILES['import']['tmp_name'], $targetFile)) {
        ...
        foreach ($data as $row) {
            ...
            $sqlRunningNo = mysqli_query($con, "SELECT * FROM aims_asset_category_run_no WHERE category LIKE '$category'");
            ...
            $sqlInsert = "INSERT INTO aims_asset 
                (name, category, date_purchase, price, supplier, branch, user, department, contact_number, location, particular, start_warranty, end_warranty, asset_tag ) VALUES (
