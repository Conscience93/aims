<?php
    ...
    $po_number = escapeInput($con, $_POST['po_number']);
    $do_number = escapeInput($con, $_POST['do_number']);

    // picture
    $view = escapeInput($con, $_POST['view']);

    // Get asset category running number
    $sql = mysqli_query($con, "SELECT * FROM aims_asset_category_run_no WHERE category = '$category'");
    ...
    for ($i = 0; $i < $quantity; $i++) {
        ...
        $sqlDuplicateCheck = "SELECT COUNT(*) as count FROM aims_asset WHERE name = '$name'";
        ...
        if (isset($_FILES['invoice']) && $_FILES['invoice']['error'] === UPLOAD_ERR_OK) {
            ...
            if (copy($invoice_tmp, $invoice_destination)) {
                ...
                $invoice = "include/upload/invoice/asset/" . $asset_tag . "/" . $invoice_name;
            }
        }
        ...
        if (isset($_FILES['document']) && $_FILES['document']['error'] === UPLOAD_ERR_OK) {
            ...
            if (copy($document_tmp, $document_destination)) {
                ...
                $document = "include/upload/document/asset/" . $asset_tag . "/" . $document_name;
            }
        }
        ...
        if (isset($_FILES['warranty']) && $_FILES['warranty']['error'] === UPLOAD_ERR_OK) {
            ...
            if (copy($warranty_tmp, $warranty_destination)) {
                ...
                $warranty = "include/upload/warranty/asset/" . $asset_tag . "/" . $warranty_name;
            }
        }

        // Insert data into aims_asset
        $sqlAsset = "INSERT INTO aims_asset 
            (name, 
            asset_tag, 
            category, 
            status, 
        ...
        // Insert data into aims_transfer_asset
        if ($queryAsset) {
            $sqlTransferAsset = "INSERT INTO aims_transfer_asset (asset_tag, name, branch, department, location) VALUES ('$asset_tag', '$name','$branch', '$department', '$location')";
            $queryTransferAsset = mysqli_query($con, $sqlTransferAsset);

            if (!$queryTransferAsset) {
                echo "Error inserting into aims_transfer_asset: " . mysqli_error($con);
                exit;
            }
        }
            ...
            for ($j = 0; $j < $pictureCount; $j++) {
                ...
                if (isset($_FILES['picture']['name'][$j]) && $_FILES['picture']['size'][$j] > 0) {
                    ...
                    if (copy($picture_tmp, $picture_destination)) {
                        ...
                        $picturesForCurrentAsset[] = "include/upload/picture/asset/" . $asset_tag . "/" . $picture_name;
                    }
                }
            }
        }
    ...
    if ($queryAsset) {
        ...
        $update_running_no = mysqli_query($con, "UPDATE aims_asset_category_run_no SET next_no = '$next_no' WHERE category = '$category'");
        echo "true";
    } else {
        echo "false";
    }
