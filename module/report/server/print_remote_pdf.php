...
// SQL query to retrieve data for software
$sqlSoftware = "SELECT aims_computer_remote_access.asset_tag, 
                        aims_computer_remote_access.remote_name, 
                        aims_computer_remote_access.remote_address, 
                        aims_computer_remote_access.remote_password, 
                        aims_computer_remote_access.remote_port
                    FROM aims_computer_remote_access ";

$resultSoftware = mysqli_query($con, $sqlSoftware);

// Store software data in an array
$softwareData = [];
while ($rowSoftware = mysqli_fetch_assoc($resultSoftware)) {
    $softwareData[] = $rowSoftware;
}

// Your existing HTML and body tags...
            ...
            <main>   
                ...
                <table class="computer-table">
                    ...
                    while ($rowUser = mysqli_fetch_assoc($resultUser)) {
                        ...
                        // Inner loop for $softwareData
                        foreach ($softwareData as $rowSoftware) {
                            if ($rowUser['asset_tag'] == $rowSoftware['asset_tag']) {
                                echo '<tr>';
                                // ... (Your existing table cells for $rowSoftware) ...
                                echo '<td>' . "RA" . '</td>';
                                echo '<td>' . ($rowSoftware['remote_name'] ? $rowSoftware['remote_name'] : '-') . '</td>';
                                echo '<td>' . ($rowSoftware['remote_address'] ? $rowSoftware['remote_address'] : '-') . '</td>';
                                echo '<td>' . ($rowSoftware['remote_password'] ? $rowSoftware['remote_password'] : '-') . '</td>';
                                echo '<td>' . ($rowSoftware['remote_port'] ? $rowSoftware['remote_port'] : '-') . '</td>';
                                echo '<td>' . "" . '</td>';
                                echo '<td>' . "" . '</td>';
                                echo '</tr>';
                            }
                        ...
                        while ($rowVirtual = mysqli_fetch_assoc($resultVirtual)) {
                            if ($rowUser['name'] == $rowVirtual['server_name']) {
                                ...
                                echo '<tr style="background-color: #cce5ff;">'; // Light shade of blue
                                echo '<td>' . "VM" . '</td>';
                                ...
                                // Inner loop for $softwareData within $resultVirtual
                                foreach ($softwareData as $rowSoftware) {
                                    if ($rowVirtual['asset_tag'] == $rowSoftware['asset_tag']) {
                                        echo '<tr>';
                                        // ... (Your existing table cells for $rowSoftware within $resultVirtual) ...
                                        echo '<td>' . "SW" . '</td>';
                                        echo '<td>' . ($rowSoftware['remote_name'] ? $rowSoftware['remote_name'] : '-') . '</td>';
                                        echo '<td>' . ($rowSoftware['remote_address'] ? $rowSoftware['remote_address'] : '-') . '</td>';
                                        echo '<td>' . ($rowSoftware['remote_password'] ? $rowSoftware['remote_password'] : '-') . '</td>';
