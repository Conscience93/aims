...
$sqlGetAsset = "SELECT * FROM aims_electronics WHERE id = '$id'";
...
if ($resultGetAsset && mysqli_num_rows($resultGetAsset) > 0) {
    ...
    if (!empty($filePath)) {
        ...
        if (!file_exists($filePath)) {
            ...
            $sqlUpdateField = "UPDATE aims_electronics SET $fieldName = '' WHERE id = '$id'";
            $resultUpdateField = mysqli_query($con, $sqlUpdateField);

            if ($resultUpdateField) {
                echo "File deleted and database updated successfully."; 
                echo "<script>window.location.href = 'http://localhost/aims/editelectronics?id=".$row["id"]."';</script>";
            } else {
    ...
    echo "No electronics record found for the provided ID.";
}
?>