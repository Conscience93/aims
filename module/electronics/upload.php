...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_FILES['import'])) {
    ...
    if (move_uploaded_file($_FILES['import']['tmp_name'], $targetFile)) {
        ...
        foreach ($data as $row) {
            ...
            $sqlRunningNo = mysqli_query($con, "SELECT * FROM aims_electronics_category_run_no WHERE category LIKE '$category'");
            ...
            $sqlInsert = "INSERT INTO aims_electronics 
                (name, category, brand, model_no, price, user, location, department, contact_number, supplier, remark, branch, asset_tag ) VALUES (
